...
#S-range for the WANS-data generation. From these WANS-data, the PDF is calculated using a Fourier transformation
global Smin   = 0.1;
global Smax   = 3.5;
global Sstep  = 0.001;

#R-range for the resulting PDF. This MUST be the same as the measurement data
global Rmin   = ;
global Rmax   = ;
global Rstep  = ;
...
function res = fun(cno, mu, beta, a3, da3, sig3, u3, eta, nu, alpha, lcc, sig1, q, cH, cN, cO, cS, dan, k, const1, const2, useQ, b, useA, density, sampleThickness, transmission, absorptionCorrection, useP, polarizedBeam, polarizationDegree, useGradient, g, useCorrAutoColl, par_r, par_delta, par_l, radiation, wavelength, s, coh, inc)
  dfactor = 1;
  scale = k;
  iObsOut = iObs(cno, mu, beta, a3, da3, sig3, u3, eta, nu, alpha, lcc, sig1, q, cH, cN, cO, cS, dan, 1, const1, const2, useQ, b, useA, density, sampleThickness, transmission, absorptionCorrection, useP, polarizedBeam, polarizationDegree, useGradient, g, useCorrAutoColl, par_r, par_delta, par_l, radiation, wavelength, s, coh, inc, Smin, Smax, Sstep, Rmin, Rmax, Rstep, dfactor, scale);
  res = iObsOut(2,:);
endfunction
...
function [stop, info] = outfun(p, optimValues, state)
  ...
  output(1, 3) = cellstr("r");
  output(1, 4) = cellstr("PDF(r)");
  output(1, 5) = cellstr("PDF(r)fit");
...
function saveFiles(name, x, y, observations, cno, mu, beta, a3, da3, sig3, u3, eta, nu, alpha, lcc, sig1, q, cH, cN, cO, cS, dan, k, const1, const2, useQ, b, useA, density, sampleThickness, transmission, absorptionCorrection, useP, polarizedBeam, polarizationDegree, useGradient, g, useCorrAutoColl, par_r, par_delta, par_l, radiation, wavelength, s, coh, inc, fitPath, id) 
  ...
  output(1, 4) = cellstr("r");
  output(1, 5) = cellstr("iObs(s)");
  output(1, 6) = cellstr("PDF(r)fit");
